...
class BlockchainController {
    ...
    submitStar() {
        this.app.post("/submitstar", async (req, res) => {
            if(req.body.address && req.body.message && req.body.signature && req.body.star) {
                ...
                try {
                    let block = await this.blockchain.submitStar(address, message, signature, star).catch(error => {console.log(error)});;
                    if(block){
                        return res.status(200).json(block);
                    } else {
                        return res.status(500).send("An error happened!");
    ...
    // This endpoint allows you to request the execution of validateChain()
    getValidateChain() {
        this.app.get("/validatechain", async (req, res) => {
            try {
                ...
                if(invalidBlocks){
                    ...
                    return res.status(404).send("There was no block");
                }
            } catch (error) {
                return res.status(500).send("An error happened!");
            }            
